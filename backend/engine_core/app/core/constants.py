...
# 雷达六柱满分「文档口径」。2026-09-17 已按 v3.0 在册 23 卡实算重写
# （原 40/33/33/31/23/14 是 39 件口径，已随裁件作废；实算脚本留在
#   .workbuddy/tmp/report_threshold_probe.py，改动后跑一遍即得）。
# ⚠ 抗压韧性只剩 4 分上限（resilience 正分只出现在 3 张在册卡），
#   报告「抗压」柱的解释力显著下降 —— 需策划决定补标定还是该柱改由 H 行为记录补强。
RADAR_CAP_IN_DOC = {
    "原则底线": 23, "执行交付": 31, "技术决策力": 19,
    "沟通协作": 18, "主动担当": 17, "抗压韧性": 4,
}

# ---------------------------------------------------------------- 大五
BIGFIVE_KEYS = ("O", "C", "E", "A", "N")   # 注意：大五的 N=情绪稳定，与六维 N=人脉不同名不同义
BIGFIVE_CN = {"O": "开放性", "C": "尽责性", "E": "外向性", "A": "宜人性", "N": "情绪稳定"}
# 2026-09-19（v3.2）由 5 下调至 3：原门槛按 44 件全量口径（39 主线+5 成就）定的，
# 24 件基准 + v3.1 复活三件后实际在册 26 件；快车道 26 件实测默认路径 O/N 仅 2 条，
# 事件覆盖已五维全 ≥5，5 条门槛在 26 件下偏苛（对应 _pending_handoff 第 5 条「实测后下调」路径）。
BIGFIVE_MIN_EVIDENCE = 3    # 行为标记 n<3 时该特质只出自评条（算法册 §6）
