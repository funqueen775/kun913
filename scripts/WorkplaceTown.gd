...
## 不用 size_changed 信号驱动：拉伸模式生效的时机和信号先后不确定，实测会拿到
## 过期尺寸（zoom 偏小 → 视野比限制范围宽 → 被 limit_right 钉住 → 左侧露灰底）。
## 每帧校对一次，尺寸没变就是两次浮点比较，开销可忽略。
...
func _update_camera(delta: float) -> void:
	...
	var target_zoom := Vector2.ONE * _cover_zoom
	if not _active_zone.is_empty():
		target_position = _active_zone["center"]
		target_zoom = Vector2.ONE * _cover_zoom * ZONE_ZOOM
